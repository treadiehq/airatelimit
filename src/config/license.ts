...
export function validateLicense(
  ...
  if (
    !forceRefresh &&
    cachedLicense &&
    now - lastValidationTime < CACHE_TTL_MS
  ) {
  ...
  const publicKeyConfigured = !LICENSE_PUBLIC_KEY.includes(
  ...
  try {
    ...
    if (Number.isNaN(expiresAt.getTime())) {
      cachedLicense = createInvalidLicense('Invalid expiration date format');
      lastValidationTime = now;
      return cachedLicense;
    }
    const nowDate = new Date();
    ...
    cachedLicense = createInvalidLicense(
